...
class MyFrame(wx.Frame):
    ...
    def button_on_button1_click(self, event):
        ...
        wx.MessageBox('文法读取成功', '成功', wx.OK | wx.ICON_INFORMATION)
        wx.MessageBox(self.output(), 'LR(1)分析表', wx.OK | wx.ICON_INFORMATION)
        wx.MessageBox(self.show(), '所有状态及对应的项目集', wx.OK | wx.ICON_INFORMATION)
    ...
    def output(self):
        s = str()
        s += '{:^100}'.format('LR(1)分析表') + '\n'
        s += '{:^30} {:^30} {:^30}'.format('状态', 'ACTION', 'GOTO') + '\n'
        s += '\n'
        s += '{:<12} '.format(' ')
        # ACTION
        for vt in self.parser.vt:
            s += '{:<12}'.format(vt)
        s += '{:<12}'.format('#')
        # GOTO
        ...
        vtx.append('#')
        # 输出每一行
        for i in range(len(self.parser.status)):
            s += '{:<12}'.format(str(i))
            for vt in vtx:
                # {0:{'b':'S1'}}
                for key in self.parser.action[i]:
                    if vt == key:
