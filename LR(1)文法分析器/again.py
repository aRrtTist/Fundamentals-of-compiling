...
class MyFrame(wx.Frame):
    def __init__(self, parent):
        ...
        self.list_ctrl.InsertColumn(1, '状态栈', wx.LIST_FORMAT_CENTER, width=250)
        self.list_ctrl.InsertColumn(2, '符号栈', wx.LIST_FORMAT_CENTER, width=250)
        self.list_ctrl.InsertColumn(3, '输入串', wx.LIST_FORMAT_CENTER, width=250)
        self.list_ctrl.InsertColumn(4, '动作说明', wx.LIST_FORMAT_CENTER, width=250)
    ...
    def output(self):
        s = str()
        s += 'LR(1)分析表'.center(95) + '\n'
        s += '状态'.center(10) + 'ACTION'.center(50) + 'GOTO'.center(50) + '\n'
        s += '\n'
        s += ' '.ljust(12)
        ...
        for i in range(len(self.parser.status)):  # 输出每一行
            ...
            for vt in vtx:
                for key in self.parser.action[i]:  # {0:{'b':'S1'}}
                    if vt == key:
                        s += self.parser.action[i][key].ljust(12)
                        break
            for vn in self.parser.vn:
                for key in self.parser.goto[i]:
                    if vn == key:
                        s += str(self.parser.goto[i][key]).ljust(12)
                        break
            s += '\n'# 换行
        return s
    ...
    def show(self):  # 显示各个状态及对应的项目集
        ...
        for key in self.parser.status.keys():
            s += str(key) + ': '
            for val in self.parser.status.values():
                s += str(val) + ' '
            s += '\n\n'
        return s

    @staticmethod
    def __output(now_step, status_stack, symbol_stack, input_str, location):
    ...
    def analyze(self):
        ...
        correct = True
        self.dict[step] = (str(step), ''.join('%s' % id for id in status_stack), ''.join('%s' % id for id in symbol_stack),''.join(input_str[location:]), '初始化')
        while correct:
            step += 1
            ...
            if find == 'acc':
                ...
                now_state = int(find[1:])
                symbol_stack.append(input_ch)
                status_stack.append(int(find[1:]))
                location += 1
                st = 'action[%s][%s]=s%s，即状态=%s入栈' % (now_state, input_ch, find[1:], find[1:])
                self.dict[step] = (str(step), ''.join('%s' %id for id in status_stack), ''.join('%s' %id for id in symbol_stack), ''.join(input_str[location:]), st)
            elif find[0] == 'r':
                num = int(find[1:])
                ...
                st = 'r%s:用%s规约，且GOTO(%s，%s)=%s入栈' % (num, g, status_stack[-2], symbol_stack[-1], find)
                self.dict[step] = (str(step), ''.join('%s' %id for id in status_stack), ''.join('%s' %id for id in symbol_stack), ''.join(input_str[location:]), st)
        if correct:
            st = "acc:分析成功"
            self.dict[step] = (str(step), ''.join('%s' %id for id in status_stack), ''.join('%s' %id for id in symbol_stack), ''.join(input_str[location:]), st)
